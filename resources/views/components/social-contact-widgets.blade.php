...
@if($whatsapp || $telegram || $messenger)
    <div class="fixed bottom-6 right-6 z-50 flex flex-col items-center gap-3">
        ...
        @if($whatsapp)
            <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $whatsapp) }}?text={{ $waText }}" target="_blank" rel="noopener noreferrer" 
               class="w-14 h-14 bg-green-500 rounded-full flex items-center justify-center text-white shadow-lg hover:shadow-2xl hover:scale-110 transition-transform duration-300 relative group">
