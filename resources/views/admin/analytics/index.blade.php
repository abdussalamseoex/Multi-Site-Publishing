<x-app-layout>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            @if (session('status'))
                <div class="mb-4 font-medium text-sm text-green-600 bg-green-100 border border-green-200 p-3 rounded-lg">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
                ...
                <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        <div class="text-gray-500 text-xs font-bold uppercase tracking-wider mb-1">Unique Users</div>
                        <div class="text-3xl font-black text-gray-800">{{ number_format($uniqueUsers) }}</div>
                    </div>
                        ...
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    @if(in_array($v->ip_address, $blockedIpList))
                                    ...
                                    @else
                                        <form method="POST" action="{{ route('admin.analytics.blockIp') }}" class="inline">
                                            ...
                                            <button type="submit" onclick="return confirm('Block this IP ({{ $v->ip_address }}) permanently?')" class="text-red-500 hover:text-red-700 bg-red-50 hover:bg-red-100 px-3 py-1 rounded text-xs font-bold transition-colors">
                                                Block IP
                                            </button>
                                        </form>
                                    @endif
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-red-50">
                            <tr>
                                <th class="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">IP Address</th>
                                <th class="px-6 py-3 text-left text-xs font-bold text-red-800 uppercase tracking-wider">Reason / Date</th>
                                <th class="px-6 py-3 text-right text-xs font-bold text-red-800 uppercase tracking-wider">Action</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-100">
                            ...
                            <tr class="hover:bg-red-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="font-mono text-red-600 font-bold">{{ $bip->ip_address }}</div>
                                </td>
                                <td class="px-6 py-4 text-sm text-gray-600">
                                    <div class="font-medium">{{ $bip->reason }}</div>
                                    <div class="text-xs text-gray-400">{{ $bip->created_at->format('M d, Y h:i A') }}</div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <form method="POST" action="{{ route('admin.analytics.unblockIp') }}">
                                        @csrf
                                        <input type="hidden" name="ip_address" value="{{ $bip->ip_address }}">
                                        <button type="submit" onclick="return confirm('Unblock {{ $bip->ip_address }}?')" class="text-green-600 hover:text-green-900 bg-green-50 hover:bg-green-100 px-3 py-1.5 rounded font-bold transition-colors">
                                            Unblock
                                        </button>
                                    </form>
                                </td>
