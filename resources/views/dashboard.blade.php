                ...
                @if(Auth::user()->role === 'admin')
                    ...
                    <a href="{{ route('admin.analytics.index') }}" class="bg-gradient-to-br from-indigo-500 to-purple-600 p-6 rounded-lg shadow-md flex items-center justify-between hover:shadow-lg transition">
                        <div>
                            ...
                            <div class="text-3xl font-black text-white mt-1">{{ number_format($stats['live_visitors']) }}</div>
                            <div class="text-xs text-indigo-200 mt-2">Today's Visits: {{ number_format($stats['visits_today']) }}</div>
                        </div>
